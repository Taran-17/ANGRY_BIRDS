...
public class LoseScreen implements Screen {
    ...
    public LoseScreen(MyAngryBirds game) {
        this.game = game;
        stage = new Stage(new ScreenViewport());
        skin = new Skin(Gdx.files.internal("uiskin.json"));  // Load default skin
        batch = new SpriteBatch();  // Initialize SpriteBatch

        // Load background texture for lose screen
        backgroundTexture = new Texture(Gdx.files.internal("lose_bg.png"));

        // Create a label for "You Lose!"
        Label loseLabel = new Label("You Lose!", skin);
        loseLabel.setFontScale(3);  // Scale the font size
        loseLabel.setPosition(Gdx.graphics.getWidth() / 2f - 100, Gdx.graphics.getHeight() - 100);  // Center the label
